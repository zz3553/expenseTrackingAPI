...
import expense_crud, schemas, db
import logging
...
def create_expense(expense: schemas.ExpenseCreate, database: Session = Depends(db.get_db)):
    return expense_crud.create_expense(database, expense)
...
def get_expenses_filtered_by_type(
    ...
    return expense_crud.get_expense_by_category(database, category)


@router.get("/{expense_id}", response_model=schemas.ExpenseResponse)
def get_expense_with_id(expense_id: int, database: Session = Depends(db.get_db)):
    expense = expense_crud.get_expense(database, expense_id)
    if not expense:
...
def update_expense(
    ...
    return expense_crud.update_expense(database, expense_id, updates)
...
def delete_expense(
    ...
    return expense_crud.delete_expense(database, expense_id)
